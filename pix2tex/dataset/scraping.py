...
def recursive_search(parser: Callable, seeds: List[str], depth: int = 2, skip: List[str] | None = None, unit: str = 'links', base_url: str = None, **kwargs) -> Tuple[List[str], List[str]]:
    ...
    """
    visited, links = set(skip or []), set(seeds)
    math = []
    try:
...
def parse_wiki(url):
    text = parse_url(url)
    linked = list(set([link for link in re.findall(wikilinks, text) if ':' not in link]))
...
def parse_stack_exchange(url):
    ...
    linked = list(
        set(
            [
                link[1]
                for link in re.findall(stackexchangelinks, text)
                if url.startswith(link[0])
            ]
        )
    )
...
def recursive_wiki(seeds, depth=4, skip=None, base_url=wiki_base):
...
def recursive_stack_exchange(seeds, depth=4, skip=None, base_url=math_stack_exchange_base):
...
if __name__ == '__main__':
    ...
    for entries, name in zip(
        [visited, math],
        [f'visited_{args.mode}.txt', f'math_{args.mode}.txt'],
        strict=True,
    ):
